...
class ChannelVideoController extends Controller
{
        ...
        try {
            ...
            if ($input_search != null && isset($input_search)) {
                ...
                if ($input_type != 0 && $input_rent == 0) {
                    ...
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)->where('type_id', $input_type)
                            ->where('channel_id', $input_channel)->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)->where('type_id', $input_type)
                    ...
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)->where('type_id', $input_type)->where('is_rent', 1)
                            ->where('channel_id', $input_channel)->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)->where('type_id', $input_type)->where('is_rent', 1)
                            ->with('type')->orderBy('id', 'desc')->paginate(15);
                    ...
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('channel_id', $input_channel)->where('video_type', 6)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                    }
                } else {
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('channel_id', $input_channel)->where('video_type', 6)
                            ->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 6)
                            ->with('type')->orderBy('id', 'desc')->paginate(15);
                    }
                }
            } else {
                ...
                if ($input_type != 0 && $input_rent == 0) {
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('channel_id', $input_channel)->where('type_id', $input_type)->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('type_id', $input_type)->with('type')->orderBy('id', 'desc')->paginate(15);
                    }
                } else if ($input_type != 0 && $input_rent == 1) {
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('channel_id', $input_channel)->where('type_id', $input_type)->where('is_rent', 1)->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('type_id', $input_type)->where('is_rent', 1)->with('type')->orderBy('id', 'desc')->paginate(15);
                    }
                } else if ($input_type == 0 && $input_rent == 1) {
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('channel_id', $input_channel)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                    }
                } else {
                    if ($input_channel != 0) {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->where('channel_id', $input_channel)->with('type')->orderBy('id', 'desc')->paginate(15);
                    } else {
                        $video_list = Video::where('producer_id', $producer['id'])->where('video_type', 6)->with('type')->orderBy('id', 'desc')->paginate(15);
                    }
                }
            }
            ...
            $type = Type::where('type', 6)->get();
            $channel = Channel::latest()->get();

            return view('producer.channel_video.index', ['result' => $video_list, 'type' => $type, 'channel' => $channel]);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('producer.channel_video.add', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function store(Request $request)
    {
        try {
            ...
            if ($org_name != null && isset($org_name)) {
                ...
                $video->thumbnail = $this->common->saveImage($org_name, $this->folder_content, 'ch_vid_');
            } elseif ($request->thumbnail_tmdb) {

                $url = $request->thumbnail_tmdb;
                $S_Name = $this->common->URLSaveInImage($url, $this->folder_content, 'ch_vid_');
                $video->thumbnail = $S_Name;
            }
            if ($org_name1 != null && isset($org_name1)) {

                $video->landscape = $this->common->saveImage($org_name1, $this->folder_content, 'ch_vid_');
    ...
    public function edit($id)
    {
        try {
            ...
            return view('producer.channel_video.edit', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function show($id)
    {
        try {
            ...
            if ($video->delete()) {
                ...
                return redirect()->route('pch_video.index')->with('success', __('label.controller.data_delete_successfully'));
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function channelVideoDetails($id)
    {
        try {
            ...
            return view('producer.channel_video.details', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
