...
@section('page_title', __('label.edit_channel_tvshow'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.edit_channel_tvshow')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('ch_tvshow.index') }}">{{__('label.channel_tv_shows')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_channel_tvshow')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('ch_tvshow.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.tvshow_list')}}</a>
                </div>
            </div>
            ...
            <form id="edit_TVShow" enctype="multipart/form-data">
                ...
                <div class="text-right">
                    ...
                    <a href="{{route('ch_tvshow.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
    ...
    <script>
        ...
        function edit_TVShow() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("ch_tvshow.update", [$result->id])}}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'edit_TVShow', '{{ route("ch_tvshow.index") }}');
                    },
