        ...
        <div class="body-content">
            ...
            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('upcomingtvshow.index') }}">{{__('label.tv_shows')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.episodes')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('upcomingtvshow.index') }}" class="btn btn-default mw-150" style="margin-top: -14px;">{{__('label.tv_shows')}}</a>
                </div>
            </div>

            <!-- Search -->
            <form action="{{ route('upcomingtvshow.episode.index', ['id' => $tvshow_id]) }}" method="GET">
            ...
            <div class="row">
                <div class="col-12 col-sm-6 col-md-4 col-xl-3">
                    <a href="{{ route('upcomingtvshow.episode.add', ['id' => $tvshow_id]) }}" class="add-video-btn" title="{{__('label.add_episode')}}">
                        ...
                        <div class="position-relative">
                            ...
                            <ul class="list-inline overlap-control" aria-labelledby="dropdownMenuLink">
                                <li class="list-inline-item">
                                    <a class="btn" href="{{ route('upcomingtvshow.episode.edit', ['tvshow_id' => $value->show_id, 'id' => $value->id])}}" title="{{__('label.edit')}}">
                                        <i class="fa-solid fa-pen-to-square fa-xl" class="dot-icon" style="color: #4e45b8;"></i>
                                    </a>
                                </li>
                                <li class="list-inline-item">
                                    <a class="btn" href="{{ route('upcomingtvshow.episode.delete', ['tvshow_id' => $value->show_id, 'id' => $value->id])}}" title="{{__('label.delete')}}" onclick="return confirm('{{__('label.delete_episode')}}')">
    ...
    <script>
        ...
        function save_episode_sortable() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("upcomingtvshow.episode.sortable") }}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'save_episode_sortable', '{{ route("upcomingtvshow.episode.index",["id" => $tvshow_id]) }}');
                    },
