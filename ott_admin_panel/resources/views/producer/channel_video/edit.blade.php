...
@section('page_title', __('label.edit_channel_video'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.edit_channel_video')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('producer.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('pch_video.index') }}">{{__('label.channel_videos')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_channel_video')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('pch_video.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.video_list')}}</a>
                </div>
            </div>
            ...
            <form enctype="multipart/form-data" id="save_edit_video">
                ...
                <div class="text-right">
                    ...
                    <a href="{{route('pch_video.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
    ...
    <script>
        ...
        function save_edit_video() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route("pch_video.update", [$result->id])}}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'save_edit_video', '{{ route("pch_video.index") }}');
                    },
