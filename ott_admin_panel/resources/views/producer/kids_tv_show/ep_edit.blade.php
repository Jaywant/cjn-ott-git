        ...
        <div class="body-content">
            ...
            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('pkidstvshow.episode.index', ['id' => $tvshow_id]) }}">{{__('label.episodes')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_episode')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('pkidstvshow.episode.index',['id'=> $tvshow_id]) }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.episode_list')}}</a>
                </div>
            </div>
            ...
            <form id="save_edit_tvshow_video" enctype="multipart/form-data">
                ...
                <div class="text-right">
                    ...
                    <a href="{{route('pkidstvshow.episode.index', ['id' => $tvshow_id])}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
	...
	<script>
		...
		function save_edit_tvshow_video() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route("pkidstvshow.episode.update", ["tvshow_id" => $tvshow_id, "id" => $result->id]) }}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'save_edit_tvshow_video', '{{ route("pkidstvshow.episode.index", ["id" => $tvshow_id]) }}');
                    },
