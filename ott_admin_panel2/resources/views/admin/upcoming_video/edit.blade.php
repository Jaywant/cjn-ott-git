...
@section('page_title', __('label.edit_upcoming_video'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.edit_upcoming_video')}}<</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('upcomingvideo.index') }}">{{__('label.upcoming_videos')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_upcoming_video')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('upcomingvideo.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.video_list')}}</a>
                </div>
            </div>
            ...
            <form enctype="multipart/form-data" id="save_edit_video">
                ...
                <div class="custom-border-card">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-6 mb-3">
                            <div class="form-group">
                                <input type="text" name="name" value="{{$result->name}}" id="Tmdb_name" list="Tmdb_name_list" class="form-control" placeholder="{{__('label.enter_movies_name')}}">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-3">
                            <div class="form-group">
                                ...
                                <input name="release_date" type="date" value="{{$result->release_date}}" id="release_date" class="form-control">
                            </div>
                        </div>
                    </div>
                    <div class="form-row">
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-4">
                            ...
                            <select name="video_upload_type" id="video_upload_type" class="form-control">
                                <option value="server_video" {{$result->video_upload_type == "server_video" ? 'selected' : ''}}>{{__('label.server_video')}}</option>
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-4">
                            ...
                            <select name="subtitle_type" id="subtitle_type" class="form-control">
                                <option value="server_video" {{ $result->subtitle_type == "server_video" ? 'selected' : ''}}>{{__('label.server_video')}}</option>
                ...
                <div class="text-right">
                    ...
                    <a href="{{route('upcomingvideo.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
    ...
    <script>
        ...
        function save_edit_video() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route("upcomingvideo.update", [$result->id])}}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'save_edit_video', '{{ route("upcomingvideo.index") }}');
                    },
        ...
        $('#Tmdb_name').keyup(function() {
            ...
            if (txtVal.length >= 3) {
                var url = "{{route('video.serach.name', '')}}" + "/" + txtVal;
                $.ajax({
                    type: "POST",
                    url: url,
                    headers: {
        ...
        $('#Tmdb_name').on('input', function() {
            ...
            $("#Tmdb_name_list").find("option").each(function() {
                if ($(this).val() == userText) {
                    ...
                    $("#dvloader").show();
                    var url = "{{route('video.getdata', '')}}" + "/" + c_id;
                    $.ajax({
                        type: "POST",
                        url: url,
                        headers: {
        ...
        function tmdb_data_fetch() {
            ...
            if (id != "") {
                ...
                $("#dvloader").show();
                var url = "{{route('video.getdata', '')}}" + "/" + id;
                $.ajax({
                    type: "POST",
                    url: url,
                    headers: {
