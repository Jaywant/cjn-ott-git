...
@section('page_title', __('label.edit_kids_video'))
...
@section('content')
    ...
    <div class="right-content">
        ...
        <div class="body-content">
            <!-- mobile title -->
            <h1 class="page-title-sm">{{__('label.edit_kids_video')}}</h1>

            <div class="border-bottom row mb-3">
                <div class="col-sm-10">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="{{ route('producer.dashboard') }}">{{__('label.dashboard')}}</a></li>
                        <li class="breadcrumb-item"><a href="{{ route('pkidsvideo.index') }}">{{__('label.kids_videos')}}</a></li>
                        <li class="breadcrumb-item active" aria-current="page">{{__('label.edit_kids_video')}}</li>
                    </ol>
                </div>
                <div class="col-sm-2 d-flex align-items-center justify-content-end">
                    <a href="{{ route('pkidsvideo.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('label.video_list')}}</a>
                </div>
            </div>
            ...
            <form enctype="multipart/form-data" id="save_edit_video">
                ...
                <div class="custom-border-card">
                    ...
                    <div class="form-row">
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <input type="text" name="Tmdb_id" id="Tmdb_id" class="form-control" placeholder="Enter TMDb ID (e.g. 814215)">
                    ...
                    <div class="form-row">
                        <div class="form-group col-lg-4">
                            ...
                            <select name="trailer_type" id="trailer_type" class="form-control">
                                <option selected="selected" value="server_video" {{ $result->trailer_type == "server_video" ? 'selected' : ''}}>{{__('label.server_video')}}</option>
                ...
                <div class="text-right">
                    ...
                    <a href="{{route('pkidsvideo.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('label.cancel')}}</a>
    ...
    <script>
        ...
        function save_edit_video() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{route("pkidsvideo.update", [$result->id])}}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'save_edit_video', '{{ route("pkidsvideo.index") }}');
                    },
        ...
        $('#Tmdb_name').keyup(function() {
            ...
            if (txtVal.length >= 3) {
                var url = "{{route('pvideo.serach.name', '')}}" + "/" + txtVal;
                $.ajax({
                    type: "POST",
                    url: url,
                    headers: {
        ...
        $('#Tmdb_name').on('input', function() {
            ...
            $("#Tmdb_name_list").find("option").each(function() {
                if ($(this).val() == userText) {
                    ...
                    $("#dvloader").show();
                    var url = "{{route('pvideo.getdata', '')}}" + "/" + c_id;
                    $.ajax({
                        type: "POST",
                        url: url,
                        headers: {
        ...
        function tmdb_data_fetch() {
            ...
            if (id != "") {
                ...
                $("#dvloader").show();
                var url = "{{route('pvideo.getdata', '')}}" + "/" + id;
                $.ajax({
                    type: "POST",
                    url: url,
                    headers: {
