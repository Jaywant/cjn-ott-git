...
class UpcomingTVShowController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                ...
                if ($input_search != null && isset($input_search)) {
                    ...
                    if ($input_type != 0 && $input_rent == 0) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->where('type_id', $input_type)
                            ->with('type')->orderBy('id', 'desc')->latest()->get();
                    } else if ($input_type != 0 && $input_rent == 1) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->where('type_id', $input_type)->where('is_rent', 1)
                            ->with('type')->orderBy('id', 'desc')->latest()->get();
                    } else if ($input_type == 0 && $input_rent == 1) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->latest()->get();
                    } else {
                        $data = TVShow::where('producer_id', $producer['id'])->where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->with('type')->orderBy('id', 'desc')->latest()->get();
                    }
                } else {

                    if ($input_type != 0 && $input_rent == 0) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('video_type', 5)->where('type_id', $input_type)->with('type')->orderBy('id', 'desc')->latest()->get();
                    } else if ($input_type != 0 && $input_rent == 1) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('video_type', 5)->where('type_id', $input_type)->where('is_rent', 1)->with('type')->orderBy('id', 'desc')->latest()->get();
                    } else if ($input_type == 0 && $input_rent == 1) {
                        $data = TVShow::where('producer_id', $producer['id'])->where('video_type', 5)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->latest()->get();
                    } else {
                        $data = TVShow::where('producer_id', $producer['id'])->where('video_type', 5)->with('type')->orderBy('id', 'desc')->latest()->get();
                    }
                }
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        ...
                        $delete = '<form onsubmit="return confirm(\'' . $tvshow_delete . '\');" method="POST"  action="' . route('pupcomingtvshow.destroy', [$row->id]) . '">
                                <input type="hidden" name="_token" value="' . csrf_token() . '">
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="edit-delete-btn" style="outline: none;" title="' . $delete . '"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around">';
                        $btn .= '<a href="' . route('pupcomingtvshow.details', [$row->id]) . '" class="edit-delete-btn mr-2" title="' . $statistics . '">';
                        $btn .= '<i class="fa-solid fa-chart-line fa-xl"></i>';
                        $btn .= '</a>';
                        $btn .= '<a href="' . route('pupcomingtvshow.edit', [$row->id]) . '" class="edit-delete-btn mr-2" title="' . $edit . '">';
                    ...
                    ->addColumn('episode', function ($row) {
                        ...
                        $btn = '<a href="' . route("pupcomingtvshow.episode.index", $row->id) . '" class="btn text-white p-1 font-weight-bold" style="background:#4e45b8;">' . $episode_list . '</a> ';
                        return $btn;
                    })
            ...
            return view('producer.upcoming_tv_show.index', $params);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('producer.upcoming_tv_show.add', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function store(Request $request)
    {
        try {
            ...
            if ($org_name != null && isset($org_name)) {
                ...
                $S_Name = $this->common->URLSaveInImage($url, $this->folder_content, 'up_show_');
    ...
    public function edit($id)
    {
        try {
            ...
            return view('producer.upcoming_tv_show.edit', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function destroy($id)
    {
        try {
            ...
            if ($TVShow->delete()) {
                ...
                return redirect()->route('pupcomingtvshow.index')->with('success', __('label.controller.data_delete_successfully'));
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    ...
    public function TVShowIndex(Request $request, $id)
    {
        try {
            ...
            return view('producer.upcoming_tv_show.ep_index', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function TVShowAdd($id)
    {
        try {
            ...
            return view('producer.upcoming_tv_show.ep_add', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function TVShowedit($show_id, $id)
    {
        try {
            ...
            return view('producer.upcoming_tv_show.ep_edit', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function TVShowDelete($show_id, $id)
    {
        try {
            ...
            if ($TVShowVideo->delete()) {
                ...
                return redirect()->route('pupcomingtvshow.episode.index', ['id' => $show_id])->with('success', __('label.controller.data_delete_successfully'));
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    ...
    public function upcomingTVShowDetails($id)
    {
        try {
            ...
            return view('producer.upcoming_tv_show.details', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
