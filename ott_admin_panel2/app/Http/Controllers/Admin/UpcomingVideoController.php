...
class UpcomingVideoController extends Controller
{
        ...
        try {
            ...
            if ($input_search != null && isset($input_search)) {
                ...
                if ($input_type != 0 && $input_rent == 0) {
                    $video_list = Video::where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->where('type_id', $input_type)
                        ->with('type')->orderBy('id', 'desc')->paginate(15);
                } else if ($input_type != 0 && $input_rent == 1) {
                    $video_list = Video::where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->where('type_id', $input_type)->where('is_rent', 1)
                        ->with('type')->orderBy('id', 'desc')->paginate(15);
                } else if ($input_type == 0 && $input_rent == 1) {
                    $video_list = Video::where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                } else {
                    $video_list = Video::where('name', 'LIKE', "%{$input_search}%")->where('video_type', 5)->with('type')->orderBy('id', 'desc')->paginate(15);
                }
            } else {
                ...
                if ($input_type != 0 && $input_rent == 0) {
                    ...
                    $video_list = Video::where('video_type', 5)->where('type_id', $input_type)->where('is_rent', 1)->with('type')->orderBy('id', 'desc')->paginate(15);
                } else if ($input_type == 0 && $input_rent == 1) {
                    $video_list = Video::where('video_type', 5)->with('type')->where('is_rent', 1)->orderBy('id', 'desc')->paginate(15);
                } else {
            ...
            return view('admin.upcoming_video.index', ['result' => $video_list, 'type' => $type, 'releases_type' => $releases_type, 'channel_list' => $channel_list]);
        } catch (Exception $e) {
    ...
    public function show($id)
    {
        try {
            ...
            if ($video->delete()) {
                ...
                return redirect()->route('upcomingvideo.index')->with('success', __('label.controller.data_delete_successfully'));
            }
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    ...
    public function upcomingVideoDetails($id)
    {
        try {
            ...
            return view('admin.upcoming_video.details', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
